...
function weatherLabel(code) {
    ...
    function getPreferredVoice() {
      return window.NeoVoiceEngine?.getPreferredVoice?.(state.prefs.lang)
        || window.NeoVoice?.getPreferredVoice?.(state.prefs.lang)
        || null;
    }
    ...
    function generateChatMeta(text) {
      return window.NeoCore?.generateChatMeta(text, getCurrentChat()?.messages || [], state.prefs.lang)
        || window.NeoChatLogic?.generateChatMeta(text, getCurrentChat()?.messages || [], state.prefs.lang) || {
        title: "Nowy czat",
    ...
    function analyzeMessageContext(text) {
      return window.NeoCore?.analyzeContext(text, getCurrentChat()?.messages || [])
        || window.NeoChatLogic?.analyzeMessageContext(text, getCurrentChat()?.messages || [])
        || { temat: "Rozmowa", opis: "Standardowa wiadomość użytkownika.", emocje: "neutralne" };
    }

    function setStatus(text) {
      const content = String(text || "").trim();
      if (!content) return;
      const localReadyTexts = Object.values(translations).map((entry) => String(entry.localReady || "").trim()).filter(Boolean);
      if (localReadyTexts.includes(content)) return;
      let type = "info";
      let title = "NeoAI";
        if (/limit|wykorzystano juz limit|ostrzezenie|warning/i.test(content)) {
          type = "warning";
          title = "Limit lub ostrzezenie";
        window.NeoVoiceTTS?.play("warning");
        } else if (/start10|aktywow|gratulacje|plus na 10 dni|plan plus/i.test(content)) {
          type = "success";
          title = "Aktywacja";
        window.NeoVoiceTTS?.play("success");
        } else {
        window.NeoVoiceTTS?.play("info");
        }
      window.NeoUIEngine?.notify?.({ type, title, text: content }) || window.NeoNotifications?.push({ type, title, text: content });
      }
  ...
  function deleteChatAnimated(chatId, itemNode) {
    if (itemNode) {
      itemNode.classList.add("removing");
      window.setTimeout(() => deleteChat(chatId), 180);
      return;
    }
    deleteChat(chatId);
  }
    ...
    function renameChat(chatId) {
      const chat = getUserChats().find((entry) => entry.id === chatId);
      if (!chat) return;
      const nextTitle = window.prompt("Nowa nazwa czatu:", chat.title || "Nowy czat");
      if (!nextTitle) return;
    ...
    function exportChat(chatId) {
      ...
      link.href = url;
      link.download = `${(chat.title || "neoai-chat").replace(/[^\w\-]+/g, "_")}.json`;
      link.click();
      URL.revokeObjectURL(url);
      setStatus(`Wyeksportowano czat: ${chat.title || "Nowy czat"}`);
    }
  ...
  function renderHistory() {
      ...
      for (const chat of chats) {
          const item = window.NeoUIEngine?.createHistoryItem?.({
            chat,
            active: chat.id === state.currentChatId && !state.draftChat,
            onOpen: () => {
          ...
          if (!item.className) item.className = "history-item";
    ...
    function buildReply(input) {
      return window.NeoCore?.buildResponse({
        input,
      ...
      }) || window.NeoChatLogic?.buildReply({
        input,
        model: el.modelSelect.value,
        lang: state.prefs.lang,
        location: state.location,
    ...
    async function streamText(text, speed, onChunk) {
      ...
      for (const part of parts) {
        current += part;
        onChunk(current);
        if (part.trim()) window.NeoVoiceTTS?.play("type");
    ...
    async function handleChatSubmit(event) {
      ...
      if (window.NeoPromo?.isPromoCode?.(input) || window.NeoPromotions?.isStart10(input)) {
        if (isGuest()) {
          openAuthModal("Zaloguj się lub zarejestruj, aby aktywować START10.");
          return;
        }
        const user = currentUser();
        (window.NeoPromo?.activateStart10?.(user) || window.NeoPromotions.applyStart10(user));
        persistCurrentUser(user);
        updateProfileUI();
        updateLimitUI();
        updateUpgradeInfo();
        window.NeoVisualFX?.triggerCelebration?.("START10 sukces");
        ...
        const assistantText = window.NeoPromo?.successMessage?.() || window.NeoPromotions.successMessage();
      ...
      try {
        ...
        window.NeoVisualFX?.reactToEmotion?.(userMessage.analysis?.emocje || "neutralne", input);
        ...
        window.NeoVisualFX?.reactToEmotion?.(assistantMessage.analysis?.emocje || "neutralne", `${input} ${assistantMessage.text}`);
        if (state.lastInputMode === "voice" || el.modelSelect.value === "neo-4.0-voice") {
            window.NeoVoiceTTS?.speak(assistantMessage.text, state.prefs.lang);
    ...
    function normalizeUser(email) {
      ...
      if (existing) {
        existing.usedCodes = Array.isArray(existing.usedCodes) ? existing.usedCodes : [];
        return existing;
      }
    ...
    function ensureUserCodes(user) {
      user.usedCodes = Array.isArray(user.usedCodes) ? user.usedCodes : [];
      return user.usedCodes;
    }

    function hasUsedCode(user, code) {
      return ensureUserCodes(user).includes(String(code || "").toUpperCase());
    }

    function markCodeUsed(user, code) {
      const normalized = String(code || "").toUpperCase();
      const used = ensureUserCodes(user);
    ...
    function handleRedeemCode() {
      ...
      const raw = el.bonusCodeInput.value.trim().toUpperCase();
        if (window.NeoPromo?.isPromoCode?.(raw) || window.NeoPromotions?.isStart10(raw)) {
          const user = currentUser();
        if (hasUsedCode(user, raw)) {
          setPanelStatus(el.settingsStatus, "error", "Ten kod został już wykorzystany na tym koncie.");
          return;
        }
        (window.NeoPromo?.activateStart10?.(user) || window.NeoPromotions.applyStart10(user));
          persistCurrentUser(user);
          ...
          updateLimitUI();
          updateUpgradeInfo();
        setPanelStatus(el.settingsStatus, "info", window.NeoPromo?.successMessage?.() || window.NeoPromotions.successMessage());
        window.NeoVisualFX?.triggerCelebration?.("START10 sukces");
          return;
      }
      ...
      const user = currentUser();
      if (hasUsedCode(user, raw)) {
        setPanelStatus(targetStatus, "error", "Ten kod został już wykorzystany na tym koncie.");
        return;
      }
      if (!match) {
    ...
    function initVoiceRecognition() {
      ...
      window.NeoVoiceTTS?.configureRecognition(state.recognition, {
        ...
        onEnd: () => {
          ...
          window.NeoVisualFX?.setListening(false);
        },
    ...
    function initState() {
      ...
      updateProfileUI();
      updateLimitUI();
        window.NeoNotifications?.clear();
        const welcomeText = window.NeoPromo?.welcomeText?.() || window.NeoApp?.welcomeMessage?.() || "Witaj w NeoAI! Wpisz kod START10, aby zgarnąć Plan Plus.";
        if (window.NeoUIEngine?.showWelcomeNotice) {
          window.NeoUIEngine.showWelcomeNotice(welcomeText);
        } else {
          window.NeoNotifications?.push({ type: "success", title: "Kod z okazji startu 🎁", text: welcomeText, key: "welcome-start10" });
        }
