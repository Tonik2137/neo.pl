    ...
    function generateChatMeta(text) {
      return window.NeoChatLogic?.generateChatMeta(text, getCurrentChat()?.messages || [], state.prefs.lang) || {
        title: "Nowy czat",
        description: "Rozmowa rozpoczeta"
      };
    }

    function analyzeMessageContext(text) {
      return window.NeoChatLogic?.analyzeMessageContext(text) || { topic: "Rozmowa", description: "Standardowa wiadomosc uzytkownika." };
    }
    ...
    function updateProfileUI() {
      ...
      el.logoutBtn.textContent = isGuest() ? "🚪 Zaloguj się" : "🚪 Wyloguj się";
    ...
    function createMessageNode(role, text, typing = false, meta = {}) {
      ...
      article.innerHTML = `
        ${avatar}
        <div class="bubble">${attachment}${typing ? '<div class="typing"><span class="typing-grid"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></span><strong>NeoAI tworzy odpowiedź/obraz...</strong></div>' : role === "assistant" ? formatAssistantMessage(text) : escapeHtml(text)}</div>
      `;
    ...
    async function handleChatSubmit(event) {
      ...
      if (attachment) {
        reply = `Analizuję Twój screen... 🔍\n\nNa miniaturce widzę plik „${attachment.name}”. To wygląda jak ${attachment.description}. Mogę teraz opisać problem, wskazać błędy i zasugerować poprawki krok po kroku.`;
      ...
      const assistantMessage = { role: "assistant", text: "", attachment: assistantAttachment, analysis: analyzeMessageContext(reply) };
    ...
    function initState() {
      ...
      state.chats = normalizeChatBuckets(storageLoad(STORAGE_KEYS.chats, {}));
      storageSave(STORAGE_KEYS.chats, state.chats);
